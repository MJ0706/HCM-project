...
class MEmodel(object):
	...
	default_params = {"EDP":12, "maxit":20, "restol":1e-3, "drestol":1e-4, "volinc":1, "LVangle":[60,-60], "V_LV_target":45.0}
	...
	V_LV_target = default_params["V_LV_target"]
	LVendoid = self.SimDet["LVendoid"]
	...
	if(MPI.rank(comm_me) == 0):
		...
		fdataMec = open(outfolder + "BiV_unloadMec.txt", "w", 0)
	...
	while(1):
		...
		#vtkfile_old  = File(outputfolder + folderName +  'Mesh/target_' + str(it)+'.pvd')
		#vtkfile_new  = File(outputfolder + folderName +  'Mesh/newmesh_' + str(it)+'.pvd')
		#vtkfile_me  = File(outputfolder + folderName +  'Mesh/mesh_me_' + str(it)+'.pvd')
		...
		#vtkfile_old << targetmesh
		#vtkfile_new <<  newmesh
		#vtkfile_me << self.mesh_me
		...
		print f0_me_Gauss.vector().array()
	...
	File(outputfolder + folderName +  'Mesh/target_' + str(it_load)+'.pvd')  << self.mesh_me
	File(outputfolder + folderName +  'Mesh/facetboundaries_' + str(it_load)+'.pvd')  << self.facetboundaries_me
	#File(outputfolder + folderName +  'Mesh/fiber_' + str(it_load)+'.vtp')  << f0_me_Gauss
	#File(outputfolder + folderName +  'Mesh/sheet_' + str(it_load)+'.vtp')  << s0_me_Gauss
	#File(outputfolder + folderName +  'Mesh/sheetnormal_' + str(it_load)+'.vtp')  << n0_me_Gauss
    ...
    def Getfstressglobal(self):

	return self.uflforms.globalfiberstress() #+ self.activeforms.fiberstress()

    def Getfstretchglobal(self):

	return self.uflforms.globalfiberstretch() #+ self.activeforms.fiberstress()
	...
	ECC = eCC/sqrt(inner(eCC, eCC))
	Ecc = project(ECC,fiberFS )

	ERR = eRR/sqrt(inner(eRR, eRR))
	Err = project(ERR,fiberFS )

	ELL = eLL/sqrt(inner(eLL, eLL))
	Ell = project(ELL,fiberFS )


	eCC.vector()[:] = Ecc.vector().array()[:]
        eRR.vector()[:] = Err.vector().array()[:]
        eLL.vector()[:] = Ell.vector().array()[:]
