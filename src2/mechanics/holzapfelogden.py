...
class HolzapfelOgden(object):
	...
	def I4f_k(self):
		...
		kappa = self.parameters["material params"]["kappa"]
	...
	def I5f_k(self):
		...
		I5fk = kappa*(pow(I1,2) - 2*I2) + (1.0-3.0*kappa)*I5
        	...
        	if diff == 0:
        	    try:
        	        if float(a) > dolfin.DOLFIN_EPS:
        	            if float(b) > dolfin.DOLFIN_EPS:
        	                return (
        	                    a / (2.0 * b) * (dolfin.exp(b * self.subplus(self.I4f_k() - 1) ** 2) - 1.0)
        	                )
        	            else:
        	                return a / 2.0 * self.subplus(self.I4f_k() - 1) ** 2
        	        else:
        	            return 0.0
        	    except Exception:
        	        # Probably failed to convert a and b to float
        	        return a / (2.0 * b) * (dolfin.exp(b * self.subplus(self.I4f_k() - 1) ** 2) - 1.0)

        	elif diff == 1:
        	    return a * self.subplus(self.I4f_k() - 1) * dolfin.exp(b * pow(self.I4f_k() - 1, 2))
        	elif diff == 2:
        	    return (
        	        a
        	        * heaviside(self.I4f_k() - 1)
        	        * (1 + 2.0 * b * pow(self.I4f_k() - 1, 2))
        	        * dolfin.exp(b * pow(self.I4f_k() - 1, 2))
        	    )
	...
	def globalpassivefstretch(self):
		...
		I4f = sqrt(kappa*I1 + (1.0-3.0*kappa)*I4)
		

		return I4f #f0[i]*Sca[i,j]*f0[j]


	def TotalPasMatSEF(self, diff=0, *args, **kwargs):

		a = self.parameters["material params"]["a"]
        	b = self.parameters["material params"]["b"]

		a_f = self.parameters["material params"]["a_f"]
        	b_f = self.parameters["material params"]["b_f"]

		a_s = self.parameters["material params"]["a_s"]
        	b_s= self.parameters["material params"]["b_s"]

		a_fs = self.parameters["material params"]["a_fs"]
        	b_fs = self.parameters["material params"]["b_fs"]
		...
		if I4f == 0:
        	...
        	if diff == 0:
        	    try:
        	        if float(a_f) > dolfin.DOLFIN_EPS:
        	            if float(b_f) > dolfin.DOLFIN_EPS:
        	                W4f =  (
        	                    a_f / (2.0 * b_f) * (dolfin.exp(b_f * self.subplus(I4f - 1) ** 2) - 1.0)
        	                )
        	            else:
        	                W4f = a_f / 2.0 * self.subplus(I4f - 1) ** 2
        	        else:
        	            return 0.0
        	    except Exception:
        	        # Probably failed to convert a and b to float
        	        W4f =  a_f / (2.0 * b_f) * (dolfin.exp(b_f * self.subplus(I4f - 1) ** 2) - 1.0)
		...
		I4s = inner(Cmat*s0, s0)
        	...
        	if diff == 0:
        	    try:
        	        if float(a_s) > dolfin.DOLFIN_EPS:
        	            if float(b_s) > dolfin.DOLFIN_EPS:
        	                W4s =  (
        	                    a_s / (2.0 * b_s) * (dolfin.exp(b_s * self.subplus(I4s - 1) ** 2) - 1.0)
        	                )
        	            else:
        	                W4s = a_s / 2.0 * self.subplus(I4s - 1) ** 2
        	        else:
        	            return 0.0
        	    except Exception:
        	        # Probably failed to convert a and b to float
        	        W4s =  a_s / (2.0 * b_s) * (dolfin.exp(b_s * self.subplus(I4s - 1) ** 2) - 1.0)
		...
		I8 = inner(Cmat*f0, s0)
        	...
        	try:
        	    if float(a_fs) > dolfin.DOLFIN_EPS:
        	        if float(b_fs) > dolfin.DOLFIN_EPS:
        	            W8 = a_fs / (2.0 * b_fs) * (dolfin.exp(b_fs * I8 ** 2) - 1.0)
        	        else:
        	            W8 = a_fs / 2.0 * I8 ** 2
        	    else:
        	        W8 = 0.0
	...
	def globalpassivefstress(self):

		u = self.parameters["displacement_variable"]
		f0 = self.parameters["fiber"]
		kappa = self.parameters["material params"]["kappa"]
		W  = self.TotalPasMatSEF(diff=0)
		d = u.geometric_dimension()
	    	I = Identity(d)
	    	F = I + grad(u)
		F = dolfin.variable(F)
		...
		Pk1 = dolfin.diff(W,F)#self.PK1()
		fstress = tr(H*Pk1)

		return fstress

	def structTensor(self):
		k = self.parameters["material params"]["kappa"]
		F = self.parameters["F"]
		f0 = self.parameters["fiber"]
