...
class MEmodel(object):
	...
	VQuadelem_me = VectorElement("Quadrature", 
        ...
        fiberFS = FunctionSpace(self.mesh_me, VQuadelem_me)
	...
	while(1):
		...
		File(outputfolder + folderName +"/Mesh/old_MESH_global_before_"+str(it)+ ".pvd") << self.Mesh.mesh
		while(1):
		...
		if(it > maxit or res < restol or dres < drestol):
			...
			File(outputfolder + folderName +"/Mesh/old_mesh_me_"+str(it)+ ".pvd") << self.mesh_me
			File(outputfolder + folderName +"/Mesh/old_MESH_global_"+str(it)+ ".pvd") << self.Mesh.mesh
			#F0_old = project(self.f0_me,fiberFS )

			#outdirectory = ""
			oldfiberfilename = outputfolder + folderName +"/fiberdir/old_f0_"+str(it)+ "_"
			#vtk_py.convertQuadDataToVTK(self.mesh_me, fiberFS, F0_old, vtkoutfile, outdirectory)
			self.GetFiberDirection(e0 = self.f0_me, fiberfilename = oldfiberfilename)
			#vtkoutfile = outputfolder + folderName +"/fiberdir/old_s0_"+str(it)+ "_"
			#vtk_py.convertQuadDataToVTK(self.mesh_me, fiberFS, self.s0_me, vtkoutfile, outdirectory)

			#vtkoutfile = outputfolder + folderName +"/fiberdir/old_n0_"+str(it)+ "_"
			#vtk_py.convertQuadDataToVTK(self.mesh_me, fiberFS, self.n0_me, vtkoutfile, outdirectory)
			...
			File(outputfolder + folderName +"/Mesh/new_mesh_"+str(it)+ ".pvd") << newmesh
			File(outputfolder + folderName +"/Mesh/new_MESH_global_"+str(it)+ ".pvd") << self.Mesh.mesh
		        #Update mesh
		        self.Reset()
			...
			File(outputfolder + folderName +"/Mesh/new_mesh_me_"+str(it)+ ".pvd") << self.mesh_me

			#F0_new = project(self.f0_me,fiberFS )

			#outdirectory = ""
			newfiberfilename = outputfolder + folderName +"/fiberdir/new_f0_"+str(it)+ "_"
			#vtk_py.convertQuadDataToVTK(self.mesh_me, fiberFS, F0_new, vtkoutfile, outdirectory)
			self.GetFiberDirection(e0 = self.f0_me, fiberfilename = newfiberfilename)
			#vtkoutfile = outputfolder + folderName +"/fiberdir/new_s0_"+str(it)+ "_"
			#vtk_py.convertQuadDataToVTK(self.mesh_me, fiberFS, self.s0_me, vtkoutfile, outdirectory)

			#vtkoutfile = outputfolder + folderName +"/fiberdir/new_n0_"+str(it)+ "_"
			#vtk_py.convertQuadDataToVTK(self.mesh_me, fiberFS, self.n0_me, vtkoutfile, outdirectory)
	...
	#File(outputfolder + folderName +"/Mesh/mesh.pvd") << self.mesh_me
	File(outputfolder + folderName +"/Mesh/facetbound.pvd") << self.facetboundaries_me
	File(outputfolder + folderName +"/Mesh/edgebound.pvd") << self.edgeboundaries_me		
	#self.GetFiberPlot(e0 = f0_me_Gauss)
	...
	#vtk_py.convertQuadDataToVTK(self.mesh_me, fiberFS, self.f0_me, vtkoutfile, outdirectory)

	#vtkoutfile = outfolder+"_e1_fiber"
	#vtk_py.convertQuadDataToVTK(self.mesh_me, fiberFS, self.s0_me, vtkoutfile, outdirectory)

	#vtkoutfile = outfolder+"_e2_fiber"
	#vtk_py.convertQuadDataToVTK(self.mesh_me, fiberFS, self.n0_me, vtkoutfile, outdirectory)

	os.system("cp " + outfolder + "UnloadMesh.hdf5" + " " + outfolder + "UnloadMesh_refine.hdf5")

	return it_load, volinc, LVV
