...
def run_BiV_ClosedLoop(IODet, SimDet): 
    ...
    VTK_mesh = File(outputfolder + IODet["caseID"] + '/' +  'Mesh/mesh_me_' +'.pvd')
    VTK_displacement = File(outputfolder + IODet["caseID"] + '/' +  'disp/disp_' +'.pvd')
	...
	#volinc_, V_LV_target = MEmodel_.unloading(unloadparam)
	...
	for it in np.arange(0,nloadstep_):
        	...
        	MEmodel_.LVCavityvol.vol += volinc_ #(V_LV_target - V_LV_unload)/nloadstep_
    ...
    # No unloading
	    ...
	    VTK_mesh << MEmodel_.mesh_me
	    VTK_displacement << MEmodel_.GetDisplacement()
