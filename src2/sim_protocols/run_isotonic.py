...
def run_isotonic(IODet, SimDet):
    ...
    if("nload" in SimDet.keys()):
		nload = SimDet["nload"]
    if("applied load" in SimDet.keys()):
		applied_load = SimDet["applied load"]
    ...
    Fdisp = File(IODet["outputfolder"] + IODet["folderName"] + IODet["caseID"] + "/u_disp.pvd")
