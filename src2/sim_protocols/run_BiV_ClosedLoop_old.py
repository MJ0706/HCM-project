...
def run_BiV_ClosedLoop(IODet, SimDet): 
	...
	File(outputfolder + folderName +  'Mesh/new_'+'.pvd')  << MEmodel_.mesh_me
	...
	for it in np.arange(0,nloadstep_):
		...
		fstretch = MEmodel_.Getfstretchglobal()
		export.writefStretch(MEmodel_, 0, fstretch)

		fstress = MEmodel_.Getfstressglobal()
		export.writefStress(MEmodel_, 0, fstress)
